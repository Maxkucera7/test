...
--
-- Počítač: 127.0.0.1
-- Vytvořeno: Stř 27. dub 2022, 09:41
...
--
-- Zástupná struktura pro pohled `cena_vsech_produktu`
-- (Vlastní pohled viz níže)
--
CREATE TABLE `cena_vsech_produktu` (
`SUM(quantityInStock * buyPrice)` decimal(37,2)
);
...
--
-- Vypisuji data pro tabulku `offices`
--
...
--
-- Vypisuji data pro tabulku `orderdetails`
--
...
--
-- Struktura tabulky `productlines`
--
...
--
-- Vypisuji data pro tabulku `productlines`
--
...
--
-- Zástupná struktura pro pohled `top_objednavaci`
-- (Vlastní pohled viz níže)
--
CREATE TABLE `top_objednavaci` (
`customerName` varchar(50)
,`Kontaktní osoba` varchar(101)
,`soucet` decimal(32,2)
,`Počet objednávek` bigint(21)
);
...
(1, 'vratislavino@gmail.com', '$2y$10$GIIO4Pp8WgLRx3K9SgEyH.QMhyuAKEB.gbbb8dXS0bLLfN6lHDFkq'),
(2, 'vratislavinoa@gmail.com', '$2y$10$JzRQywTFGy3cYutBTy2DGu8pZHo2bhV9YX2BPCSNGFjwFIn8Gu8Fm'),
(3, 'vratislavino2@gmail.com', '$2y$10$yQjaN9pc92J7KrlXdRpK5O8oWRxj.SDUUkY1369biX4AQslaO3wnq'),
(4, 'vratislavino3@gmail.com', '$2y$10$NfxJhIZAS5g7eBpX2qetpOUWNUwm5xVRGAGk4THPpKO6bSlfZN.yC');

-- --------------------------------------------------------

--
-- Struktura pro pohled `cena_vsech_produktu`
--
DROP TABLE IF EXISTS `cena_vsech_produktu`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `cena_vsech_produktu`  AS SELECT sum(`products`.`quantityInStock` * `products`.`buyPrice`) AS `SUM(quantityInStock * buyPrice)` FROM `products` ;

-- --------------------------------------------------------

--
-- Struktura pro pohled `top_objednavaci`
--
DROP TABLE IF EXISTS `top_objednavaci`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `top_objednavaci`  AS SELECT `c`.`customerName` AS `customerName`, concat(`c`.`contactFirstName`,' ',`c`.`contactLastName`) AS `Kontaktní osoba`, sum(`p`.`amount`) AS `soucet`, count(`p`.`amount`) AS `Počet objednávek` FROM (`payments` `p` join `customers` `c` on(`c`.`customerNumber` = `p`.`customerNumber`)) GROUP BY `p`.`customerNumber` ORDER BY sum(`p`.`amount`) DESC ;
...
--
-- Indexy pro tabulku `productlines`
--
